...
function scrapeMako(siteUrl, callback) {
    ...
    function setTitleAndDescription($, scope) {
        recipe.name = $('h1[itemprop=name]', scope).text();
        recipe.description = $('h2[itemprop=summary]', scope).text();
    }

    function setInstructions($, scope) {
        var instructions = []

        $(".recipeInstructions", scope).children().each(function (i, elem) {
    ...
    function setIngredients($, scope) {
        ...
        $("ul.recipeIngredients", scope).each(function (i, elem) {
